...
from .decoder_controlnet import Decoder, DecoderOutput, DiagonalGaussianDistribution, Encoder, zero_module, conv_nd
...
class AutoencoderKL_ControlNet_Decoder(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.zero_conv_out = self.make_zero_conv(latent_channels) # Jaihoon 2023.07.09

    def get_parameters(self):
        _params = []
        # Down blocks
        _params += list(self.decoder.get_parameters())
    ...
    def _decode(self, z: torch.FloatTensor, 
        ...
        dec, outs = self.decoder(z, src_latent=src_latent, control_config=control_config)
    ...
    def decode(self, z: torch.FloatTensor,
        ...
        if self.use_slicing and z.shape[0] > 1:
            decoded_slices, outs = [self._decode(z_slice, src_latent) for z_slice in z.split(1)]
            decoded = torch.cat(decoded_slices)
        else:
            decoded, outs = self._decode(z, src_latent, control_config)
    ...
    def forward(self, 
        ...
        outs, sample = self.encoder(x)
        moments = self.quant_conv(sample)

        distribution = DiagonalGaussianDistribution(moments)
        posterior = AutoencoderKLOutput(latent_dist=distribution)

        if sample_posterior:
            z = posterior.latent_dist.sample(generator=generator)
        else:
            z = posterior.latent_dist.mode()

        outs.append(self.zero_conv_out(z))
